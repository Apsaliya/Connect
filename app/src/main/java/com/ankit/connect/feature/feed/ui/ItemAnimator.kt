...
class ItemAnimator: DefaultItemAnimator() {
  ...
  override fun animateAdd(viewHolder: RecyclerView.ViewHolder): Boolean {
      if (viewHolder.layoutPosition > lastAddAnimatedItem) {
        lastAddAnimatedItem++
        runEnterAnimation(viewHolder as FeedAdapter.ViewHolder)
        return false
    }
  ...
  private fun runEnterAnimation(holder: FeedAdapter.ViewHolder) {
  ...
  override fun animateChange(oldHolder: RecyclerView.ViewHolder,
    ...
    if (preInfo is FeedItemHolderInfo) {
      val holder = newHolder as FeedAdapter.ViewHolder
      val post = holder.itemView.tag as Post
      
      animateHeartButton(holder)
      //updateLikesCounter(holder, post.likesCount.toInt())
      if (FeedAdapter.ACTION_LIKE_IMAGE_CLICKED == preInfo.updateAction) {
  ...
  private fun animateHeartButton(holder: FeedAdapter.ViewHolder) {
  ...
  private fun animatePhotoLike(holder: FeedAdapter.ViewHolder) {
  ...
  private fun dispatchChangeFinishedIfAllAnimationsEnded(holder: FeedAdapter.ViewHolder) {
  ...
  private fun resetLikeAnimationState(holder: FeedAdapter.ViewHolder) {
